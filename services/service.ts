...
export const getSMAPoints = async (symbol: string) => {
  const response = await fetch(
    `https://www.alphavantage.co/query?function=SMA&symbol=${
      symbol === "" ? "IBM" : symbol
    }&interval=daily&time_period=10&series_type=close&apikey=${process.env.NEXT_PUBLIC_API_KEY}`
  );
  const data = await response.json();
  return data;
};
...
export const getRSIPoints = async (symbol: string) => {
  const response = await fetch(
    `https://www.alphavantage.co/query?function=RSI&symbol=${
      symbol === "" ? "IBM" : symbol
    }&interval=weekly&time_period=10&series_type=open&apikey=${process.env.NEXT_PUBLIC_API_KEY}`
  );
  const data = await response.json();
  return data;
};
